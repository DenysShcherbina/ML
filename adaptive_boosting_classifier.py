...
for n in range(T):
    # Create and fit DecisionTree with calculated weight
    ...
    N = np.sum(np.abs(train_labels - predicted) / 2) / XN   # calculate the proportion of misclassifications
    # calculate the weight for the current algorithm
    alfa.append(0.5 * np.log((1 - N) / N) if N != 0 else np.log((1-1e-8) / 1e-8))

    # recalculate objects weights
...
print(f"Number of errors on the training set: {int(N)} when composing has {T} decision trees")
